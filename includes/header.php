
<!doctype html>
<html lang="ca"> <!-- meta idioma -->

<head>
    <meta charset="utf-8"><!--meta joc de caràcters -->
    <meta name="viewport" content="width=device-width, initial-scale=1"><!-- etiqueta per HTML responsive -->
    <meta name="description" content="">
    <meta name="author" content="Aleix Leon">

<title><?php echo $titol; ?></title><!-- títol de la web -->

<!-- estils aleon -->
<link rel="stylesheet" type="text/css" href="css/aleon.css" >

<!-- CSS interns --><style></style>
</head>

<body>

<!-- container_________________________________________________________________-->
<div class= 'container'>

<!-- menú capçalera____________________________________________________________-->
<header>
  <div class = 'marca'>    
    <a class='a_logo' href="index.php">
        <h2 class='logo'>02-PHP-MVC <span class='blau'>{<span class='negre'>aleon</span>}</span></h2>
    </a>
  </div>
  <nav>
    <!--<a class='button' href='test.php' >Test</a>-->
    <a class='button' href='index.php?controller=user&action=mostrarTots' >Users</a>
    <a class='button' href='index.php?controller=nota&action=mostrarTots' >Notes</a>
    <a class='button' href='index.php?controller=nota&action=crear' >Crear</a>
    <a class='button' href='about.php' >About</a>
  </nav>

    <!-- fi menú cap __________________________________________________________-->
</header>
